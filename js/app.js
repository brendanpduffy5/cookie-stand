...
CookieStore.prototype.render = function(tableElem) {
  ...
  for(let i=0; i < this.cookiesPerHour.length, i++;){
    let cellElem = document.createElement('td');
    cellElem.textContent = this.cookiesPerHour[i][2];
    rowElem.appendChild(cellElem);
  }
  cellElem = document.createElement('td');
  cellElem.textContent = this.total;
  rowElem.appendChild(cellElem);
...
function handleSubmit(event){
  ...
  let newStoreCity = event.target.newStoreCity.value;
  let newCustomerNumber = event.target.newCustomerNumber.value;
  let newCookiesNumber = event.target.newCookiesNumber.value;
  let newHours = event.target.newHours.value;

  let addedStore = new CookieStore(newStoreCity, newCustomerNumber, newCookiesNumber, newHours);

  addedStore.getCookieNumber();
  addedStore.render();
